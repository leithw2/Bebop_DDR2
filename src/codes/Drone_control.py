...
class Drone():
    ...
    def new_pos(self, x,y,theta):
        ...
        self.positions.append(Pose(Point(x,y,2), Quaternion(orientation[0],orientation[1],orientation[2],orientation[3])))
    ...
    def set_actual_depth(self, depth):
    ...
    def update(self, callback):
        ...
        if callback.camera == callback:
            ...
            # cv2.waitKey(2)
            cv2.imshow('camera view',self.video_output)
            #cv2.imshow('depth view',self.get_actual_depth())
            cv2.waitKey(1)
            print("camera callback")

        if callback.depth == callback:
    ...
    def depth_callback(self, data):
        ...
        self.set_actual_depth(cv_image_norm)
        self.update(callback.depth)
        #cv2.imshow('depth view',self.get_actual_depth())
        #cv2.waitKey(1)
...
if __name__ == '__main__':
    rospy.init_node('Nav_control', anonymous=False)
    rospy.loginfo('ProgramStarted_Nav_control_node')
    try:
        reader = Drone()
        rospy.spin()
